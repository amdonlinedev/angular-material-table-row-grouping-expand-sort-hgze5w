...
export class AppComponent implements OnInit {
  ...
  groupHeaderClick(row) {
    ...
       this._allGroup.forEach((item) => {
           const rootGroup = new Group();
           rootGroup.expanded = true;     
           const result = this.getSublevelNew(    
             this._allGroup,     
             this.allData,     
             0,     
             this.groupByColumns,     
      ...
           const dt = this.allData.filter(function(dataItem){     
             return dataItem.workitem === item.workitem;     
           });
  ...
  getGroupList(
    ...
    groups = groups.sort((a: Car, b: Car) => {
      const isAsc = 'asc';
      return this.compare(a.workitem, b.workitem, isAsc);
    });
  ...
  getSublevelNew(
    ...
    allGroup.forEach((group) => {
      ...
      if (group.workitem == dataRow.workitem.toString()) {
